...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 pool_kernel_size=(3, 2, 2), pool_type='max', basic_module=DoubleConv, conv_layer_order='cr',
                 num_groups=8):
        super(Encoder, self).__init__()
        assert pool_type in ['max', 'avg']
        if apply_pooling:
            if pool_type == 'max':
                self.pooling = nn.MaxPool3d(kernel_size=pool_kernel_size, stride=(2,2,2),padding=(1,0,0))
            else:
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,
                 scale_factor=(2, 2, 2), basic_module=DoubleConv, conv_layer_order='crg',outpadding=(1,1,1), num_groups=8):
    ...
    def forward(self, encoder_features, x):
        if self.upsample is None:
            ...
            x = self.upsample(x)
            #print("after upsample shape is:", x.shape)
            x += encoder_features
...
class UNetUpBlock(nn.Module):
    def __init__(self, in_size, out_size):
        ...
        self.conv_block = UNetConvBlock(in_size//2, out_size)
